seg.lm.fit <-function (y, XREG, Z, PSI, w, offs, opz, return.all.sol = FALSE, min_obs = min_obs)
{
    useExp.k = TRUE
    ...
    far.psi <- function(Z, PSI, id.psi.group, ret.id = TRUE, fc = 0.93, min_obs = min_obs) {
    ...
    if (!far.psi(Z, PSI, id.psi.group, FALSE, min_obs = min_obs))
        stop("seg.lm psi starting values too close each other or at the boundaries. Please change them (e.g. set 'quant=TRUE'
