seg.Ar.fit.boot<-function(obj, XREG, Z, PSI, opz, n.boot=10, size.boot=NULL, jt=FALSE,
    nonParam=TRUE, random=FALSE, break.boot=n.boot, min_obs = min_obs){
...
#-------------
      ...
      opz1$it.max <-0
      n<-nrow(Z)
      o0<-try(suppressWarnings(seg.Ar.fit(obj, XREG, Z, PSI, opz, min_obs = min_obs)), silent=TRUE)
      rangeZ <- apply(Z, 2, range) #serve sempre
      alpha <- opz$alpha
      limZ <- apply(Z, 2, quantile, names = FALSE, probs = c(alpha, 1 - alpha))
      
      if(!is.list(o0)) {
          o0<- seg.Ar.fit(obj, XREG, Z, PSI, opz, return.all.sol=TRUE, min_obs = min_obs)
          o0<-extract.psi(o0)
          ss00<-opz$dev0
      ...
      if(is.list(o0)){
        ...
        if(!nonParam) fitted.ok<-fitted(o0)
          ...
          if(random) {
            ...
            o0<-try(suppressWarnings(seg.Ar.fit(obj, Z, PSI1, opz1, min_obs = min_obs)), silent=TRUE)
            ss00<-o0$SumSquares.no.gap
          } else {
      ...
      for(k in seq(n.boot)){
        ...
        if(nonParam){
            ...
            o.boot<-try(suppressWarnings(seg.Ar.fit(obj, XREG[id,,drop=FALSE], Z[id,,drop=FALSE], PSI[id,,drop=FALSE], opz.boot, min_obs = min_obs)), silent=TRUE)
          } else {
...
##---->     o.boot<-try(seg.lm.fit(yy, XREG, Z.orig, PSI, weights, offs, opz.boot), silent=TRUE)
            ...
            o.boot<- try(suppressWarnings(seg.Ar.fit(obj, XREG, Z.orig, PSI, opz.boot, min_obs = min_obs)), silent=TRUE)
          }
          if(is.list(o.boot)){
          ...
          o <- try(suppressWarnings(seg.Ar.fit(obj, XREG, Z.orig, PSI, opz, return.all.sol=TRUE, min_obs = min_obs)), silent=TRUE)
          if(!is.list(o) && random){
              est.psi0<-apply(limZ,2,function(r) runif(1,r[1],r[2]))
              PSI1 <- matrix(rep(est.psi0, rep(nrow(Z), length(est.psi0))), ncol = length(est.psi0))
              o <- try(suppressWarnings(seg.Ar.fit(obj, XREG, Z, PSI1, opz1, min_obs = min_obs)), silent=TRUE)
      ...
      if(is.null(o0$obj)){
          PSI1 <- matrix(rep(est.psi0, rep(nrow(Z), length(est.psi0))), ncol = length(est.psi0))
          o0 <- try(suppressWarnings(seg.Ar.fit(obj, XREG, Z, PSI1, opz1, min_obs = min_obs)), silent=TRUE)
