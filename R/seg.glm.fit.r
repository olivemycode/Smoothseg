seg.glm.fit<-function(y,XREG,Z,PSI,w,offs,opz,return.all.sol=FALSE, min_obs = min_obs, ...){
#-------------------------
    useExp.k=TRUE
    #-----------------
    ...
    in.psi<-function(LIM, PSI, ret.id=TRUE){
        ...
        isOK}
    #------------
    far.psi<-function(Z, PSI, id.psi.group, ret.id=TRUE, fc=.93, min_obs = min_obs) {
    ...
    if(!far.psi(Z,PSI,id.psi.group,FALSE, min_obs = min_obs))
      stop("seg.glm psi starting values too close each other or at the boundaries. Please change them (e.g. set 'quant=TRUE'
    ...
    while (abs(epsilon) > toll) {
        ...
        id.psi.far <-far.psi(Z, PSI, id.psi.group, TRUE, fc=opz$fc, min_obs = min_obs)
