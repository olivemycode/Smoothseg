...
#-------------
  ...
  # 
    ...
    visualBoot<-opz$visualBoot
      ...
      opz1$it.max <-0
      n<-length(y)
      o0<-try(suppressWarnings(seg.lm.fit(y, XREG, Z, PSI, w, offs, opz, return.all.sol=FALSE, min_obs = min_obs)), silent=TRUE)
      ...
      #browser()
      if(!is.list(o0)) {
          o0<- suppressWarnings(seg.lm.fit(y, XREG, Z, PSI, w, offs, opz, return.all.sol=TRUE, min_obs = min_obs))
          o0<-extract.psi(o0)
          ss00<-opz$dev0
      ...
      if(is.list(o0)){
        ...
        if(!nonParam) fitted.ok<-fitted(o0)
          ...
          if(random) {
            ...
            o0<-try(suppressWarnings(seg.lm.fit(y, XREG, Z, PSI1, w, offs, opz1, min_obs = min_obs)), silent=TRUE)
            ss00<-o0$SumSquares.no.gap
          } else {
...
#     if(visualBoot) cat(0, " ", formatC(opz$dev0, 3, format = "f"),"", "(No breakpoint(s))", "\n")
      ...
      for(k in seq(n.boot)){
        ...
        o<-try(suppressWarnings(seg.lm.fit(y, XREG, Z.orig, PSI, w, offs, opz, return.all.sol=TRUE, min_obs = min_obs)), silent=TRUE)
        if(!is.list(o) && random){
                est.psi0<-apply(limZ,2,function(r)runif(1,r[1],r[2]))
                PSI1 <- matrix(rep(est.psi0, rep(nrow(Z), length(est.psi0))), ncol = length(est.psi0))
                o<-try(suppressWarnings(seg.lm.fit(y, XREG, Z, PSI1, w, offs, opz1, min_obs = min_obs)), silent=TRUE)
