seg.num.fit <-function (y, XREG, Z, PSI, w, opz, return.all.sol = FALSE) {
    ...
    far.psi <- function(Z, PSI, id.psi.group, ret.id = TRUE, fc = 0.93, min_obs = 2) {
    ...
    if (!far.psi(Z, PSI, id.psi.group, FALSE))
      stop("num psi starting values too close each other or at the boundaries. Please change them (e.g. set 'quant=TRUE'
