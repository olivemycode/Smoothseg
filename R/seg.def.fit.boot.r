...
#-------------
      ...
      opz1$it.max <-0
      n<-nrow(mfExt)
      o0<-try(suppressWarnings(seg.def.fit(obj, Z, PSI, mfExt, opz, min_obs = min_obs)), silent=TRUE)
      ...
      if(!is.list(o0)) {
          o0<- seg.def.fit(obj, Z, PSI, mfExt, opz, return.all.sol=TRUE, min_obs = min_obs)
          o0<-extract.psi(o0)
      ...
      if(is.list(o0)){
        ...
        if(!nonParam) fitted.ok<-fitted(o0)
          ...
          if(random) {
            ...
            o0<-try(suppressWarnings(seg.def.fit(obj, Z, PSI1, mfExt, opz1, min_obs = min_obs)), silent=TRUE)
            ss00<-o0$SumSquares.no.gap
          } else {
      ...
      for(k in seq(n.boot)){
        ...
        if(length(diff.selected.ss)>=(n.boot.rev-1) && all(round(diff.selected.ss[1:(n.boot.rev-1)],6)==0)){
          ...
          if(nonParam){
              ...
              o.boot<-try(suppressWarnings(seg.def.fit(obj, Z[id,,drop=FALSE], PSI[id,,drop=FALSE], mfExt[id,,drop=FALSE], opz.boot, min_obs = min_obs)), silent=TRUE)
...
##---->              o.boot<-try(seg.lm.fit(yy, XREG, Z.orig, PSI, weights, offs, opz.boot), silent=TRUE)
                    ...
                    o.boot<- try(suppressWarnings(seg.def.fit(obj, Z.orig, PSI, mfExt, opz.boot, min_obs = min_obs)), silent=TRUE)
          }
          if(is.list(o.boot)){
            ...
            o <- try(seg.def.fit(obj, Z.orig, PSI, mfExt, opz, return.all.sol=TRUE, min_obs = min_obs), silent=TRUE)
            if(!is.list(o) && random){
                est.psi0<-apply(limZ,2,function(r)runif(1,r[1],r[2]))
                PSI1 <- matrix(rep(est.psi0, rep(nrow(Z), length(est.psi0))), ncol = length(est.psi0))
            o <- try(suppressWarnings(seg.def.fit(obj, Z, PSI1, mfExt, opz1, min_obs = min_obs)), silent=TRUE)
