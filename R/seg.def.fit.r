seg.def.fit<-function (obj, Z, PSI, mfExt, opz, return.all.sol = FALSE, min_obs = min_obs) {
    useExp.k = TRUE
    ...
    if (!far.psi(Z, PSI, id.psi.group, FALSE, min_obs = min_obs))
      stop("def psi starting values too close each other or at the boundaries. Please change them (e.g. set 'quant=TRUE'
