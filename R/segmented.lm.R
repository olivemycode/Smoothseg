...
#    psi.values <- NULL
    ...
    if(n.boot<=0){
    obj<-seg.lm.fit(y,XREG,Z,PSI,weights,offs,opz, min_obs = min_obs)
    } else {
    obj<-seg.lm.fit.boot(y, XREG, Z, PSI, weights, offs, opz, n.boot=n.boot, size.boot=size.boot, random=random, break.boot=break.boot, min_obs = min_obs)
    seed<- obj$seed
      }
    if(!is.list(obj)){
