#' @export
`segmented.glm` <-
function(obj, seg.Z, psi, npsi, fixed.psi=NULL, control = seg.control(), model = TRUE, keep.class=FALSE, min_obs = min_obs, ...) {
  #########====================END  SE PSI FIXED
  ...
  #########====================END  SE PSI FIXED
    ...
    if(n.boot<=0){
      obj<-seg.glm.fit(y, XREG, Z, PSI, weights, offs, opz, min_obs = min_obs)
    } else {
      obj<-seg.glm.fit.boot(y, XREG, Z, PSI, weights, offs, opz, n.boot=n.boot, size.boot=size.boot, random=random, break.boot=break.boot, min_obs = min_obs) #jt, nonParam
      seed<- obj$seed
    }
    if(!is.list(obj)){
