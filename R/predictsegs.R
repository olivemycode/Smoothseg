...
predictsegs <- function(df, w, y, k, z = "bic", mobs = NULL, g = 1, interest = c(), col = "red", linet = "dashed", date = FALSE, ar = FALSE, p = 1, d = 0, q = 0){
  ...
  for (i in 1:length(k)){
    ...
    predseg <- solgmented(predfit, seg.Z = ~ x, Kmax = k[i], type = z, th = mobs, G = g) # Use solgmented function to predict changepoints with max k
